...
At a typical emergency department prevalence of 10% and the model's measured
performance (Sensitivity 94%, Specificity 94.5%):

| 1,000 patients | AI Positive | AI Negative |
|---|---|---|
| True ICH (100) | **94 detected** | 6 missed |
| No ICH (900) | 50 false alarms | **850 correctly cleared** |

PPV = 94 / (94 + 50) = **65%** — roughly 1 in 3 AI-positive flags is a false alarm.
NPV = 850 / (850 + 6) = **99.3%** — a negative AI result is highly reliable.
Pain Index = 50 / 94 = **0.53** — the radiologist overrides the AI on roughly
1 in every 2 true detections found.

These values shift substantially with prevalence.  The worklist viewer
recalculates PPV, NPV, and Pain Index dynamically based on the assumed local
prevalence for each study's clinical indication.
...
At typical emergency department prevalence (5–15%), the NPV exceeds 99%.
A negative AI result carries very high confidence that ICH is absent.
The routine read proceeds without modification to workflow priority.

**PPV — honest about the cost of false positives.**
At the same prevalence, PPV ranges from roughly 50–75%.  Approximately 1 in
3 AI-positive flags will not be confirmed by the radiologist.  This is not a
model failure — it is the mathematical consequence of applying a powerful test
to a rare condition.  The same principle applies to any screening test for a
low-prevalence disease, and is a principle covered in every medical school
curriculum.  The PPV is recalculated dynamically for the specific prevalence
of each patient's clinical context.
...
under clinical time pressure.  A Pain Index of 0.5 means the radiologist
encounters one false positive for every two true detections — a manageable
burden if the consequence of a missed true positive is a delayed diagnosis of
